    ...
    subSkill: {
      ...
      tag: {
        ...
        async content(event, trigger, player) {
          const cards2 = player.getCards("h", (card) => card.hasGaintag("potshanxi_tag"));
          if (cards2.length) {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        ai(target) {
          ...
          return get.effect(target, { name: "guohe_copy", position: "h" }, player2, player2) * (target.countCards("h") - target.countCards("h", (card) => card.hasGaintag("potshanxi_tag")));
        }
      }).forResult();
    },
    async content(event, trigger, player) {
      ...
      const result = await player.choosePlayerCard({
        ...
        ai(button) {
          ...
          if (player2 == target2 && button.link.name == "sha") {
            return 1;
          }
      ...
      if (result?.bool && result.links?.length) {
        ...
        target.addGaintag(cards2, "potshanxi_tag");
        if (get.name(cards2[0]) == "sha") {
          player.when({ player: "potqizhouAfter" }).filter((evt) => evt != event).step(async (event2, trigger2, player2) => {
            await player2.draw({ num: 2, gaintag: ["potshanxi_tag"] });
          });
        }
      }
    },
